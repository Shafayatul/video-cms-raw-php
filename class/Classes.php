...
class Classes{
    ...
    public function updateClasses($class_name,$category_name,$instructor_name,$class_img,$class_date_time,$classdescription,$class_id){
        ...
        if($image_size==false){
            $sql = "UPDATE $this->table_name SET class_name='$class_name', instructor_id='$instructor_name',category_id='$category_name',class_description='$classdescription',date_time='$class_date_time', updated_at = '$datetime' WHERE id =$class_id";
            // print_r($sql);exit;
            $result = mysqli_query($this->connection, $sql);
            return $result;
        }else{
            ...
            {
                if (file_exists(("../uploads/class/".$row['class_img']))) {
                   unlink("../uploads/class/".$row['class_img']);
                }
            } 
